...
validator.validateClassification = () => {
    return[
        ...
        .custom(async (classification_name) => {
            const existingclassification = await model.checkClassificationExists (classification_name)
...
//validate rules for the add inventory forms
...
//validate the inventory data and return errors if there is any
validator.validateInventoryResult = async function (req, res, next) {
    const {inv_make, inv_model, inv_year, inv_color, inv_price, inv_description, inv_miles, inv_image, inv_thumbnail, classification_id} = req.body;
    let nav = await utilities.getNavigations();
...
//validate the inventory updated data and return errors if there is any
validator.validateInventoryUpdateResult = async function (req, res, next) {
    const {inv_id,inv_make, inv_model, inv_year, inv_color, inv_price, inv_description, inv_miles, inv_image, inv_thumbnail, classification_id} = req.body;
    let nav = await utilities.getNavigations();
    let classifications = await utilities.buildClassificationList();
    let error = [];
    error = validationResult(req);
    ...
    //check for error if any redirect to the update inventorty view
    if(!error.isEmpty()){
